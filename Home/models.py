...
class ProfileManager(models.Manager):
    ...
    def get_all_profiles_is_friend(self, sender):
        profiles = Profile.objects.all().exclude(user=sender)
        profile = Profile.objects.get(user=sender)
        qs = Relationship.objects.filter(Q(sender=profile) | Q(receiver=profile))
...
class Profile(models.Model):
    ...
    bio = models.TextField(default="No bio...", max_length=300, null=True, blank=True)
    ava_pic = models.ImageField(default='default_ava.png', upload_to="profile")
    background_pic = models.ImageField(default='default_background.png', upload_to="profile")
